...
PLOT_DIR_VL = PLOT_DIR_THESIS / "vl"
PLOT_DIR_SENS = PLOT_DIR_THESIS / "sensitivity"
PLOT_DIR_IMMUN = PLOT_DIR_THESIS / "immunization"
PLOT_DIR_REGRESSION = PLOT_DIR_THESIS / "regression"
PLOT_DIR_RESUB = Path("..", "paper", "20231116-resubmission", "figures")
PLOT_DIR_RESUB2 = Path("..", "paper", "20240316-resubmission", "figures")
PLOT_DIR_RESUB2_2 = Path("..", "paper", "20240316-resubmission-2", "figures")
PLOT_DIR_N_VL = PLOT_DIR_THESIS / "N_RNA"
...
def returnPlotDir():
    return PLOT_DIR_THESIS


def returnPlotDirSymptoms():
    return PLOT_DIR_SYMPTOMS


def returnPlotDirSensitivity():
    return PLOT_DIR_SENS


def returnPlotDirViralLoad():
    return PLOT_DIR_VL


def returnPlotDirRegression():
    return PLOT_DIR_REGRESSION


def returnPlotDirImmunization():
    return PLOT_DIR_IMMUN


def returnPlotDirNRNA():
    return PLOT_DIR_N_VL


def returnPlotDirResubmission():
    return PLOT_DIR_RESUB


def returnPlotDirResubmission2():
    return PLOT_DIR_RESUB2


def returnPlotDirResubmission2_2():
    return PLOT_DIR_RESUB2_2


# Function to adjust saturation and lightness
def adjustColor(color, saturation_factor=1.2, lightness_factor=1):
    # Convert hex to RGB
...
def setCustomTheme():
    sns.set_theme(
        style="ticks",  # Use 'ticks' style for a cleaner look
        rc={
            "figure.facecolor": "white",  # Set figure background to white
            "axes.facecolor": "white",  # Set axis background to white
            "axes.edgecolor": "lightgray",  # Set axis edge color to light gray
            "grid.color": "lightgray",  # Set grid color to light gray
            "axes.grid": True,  # Ensure grid is visible
            "axes.grid.axis": "both",
            "axes.grid.which": "both",
            "axes.linewidth": 0,  # Remove axis outline
            "xtick.direction": "out",  # Set x-ticks direction to out
            "ytick.direction": "out",  # Set y-ticks direction to out
            "xtick.major.size": 4,  # Set major x-tick size
            "ytick.major.size": 4,  # Set major y-tick size
            "xtick.major.width": 1,  # Slightly thinner major ticks
            "ytick.major.width": 1,  # Slightly thinner major ticks
            "xtick.minor.size": 0,  # Remove minor x-ticks
            "ytick.minor.size": 0,  # Remove minor y-ticks
            "axes.titlesize": TITLE_SIZE,  # Set plot title size
            "axes.labelsize": TEXT_SIZE,  # Set axis label size
            "xtick.labelsize": TEXT_SIZE,  # Set x-tick label size
            "ytick.labelsize": TEXT_SIZE,  # Set y-tick label size
            "xtick.color": "lightgray",  # Ensure tick lines are black
            "ytick.color": "lightgray",  # Ensure tick lines are black
            "xtick.labelcolor": "#4A4A4A",  # Set x-tick label color to
            # dark gray
            "ytick.labelcolor": "#4A4A4A",  # Set y-tick label color to
            # dark gray
            "legend.title_fontsize": LEGEND_TITLE_SIZE,  # Set legend title size
            "legend.fontsize": LEGEND_TEXT_SIZE,  # Set legend text size
            "axes.spines.right": False,  # Remove right spine
            "axes.spines.top": False,  # Remove top spine
            "axes.spines.left": False,  # Remove left spine
            "axes.spines.bottom": False,  # Remove bottom spine
            "patch.linewidth": 0.5,  # Thinner patch lines
            "legend.handlelength": LEGEND_KEY_SIZE / 4,  # Adjust legend key size
            "font.family": "sans-serif",  # Use a slimmer sans-serif font
            "font.sans-serif": "Arial",
            "font.weight": "light",
        },
    )
    # Adjust the colors in the custom palette
    adjustedPalette = [adjustColor(color) for color in GGPLOT_PALETTE]
    sns.set_palette(adjustedPalette)
...
def annotateWithLetter(ax, letter, size=20, coords=(-0.175, 1)):
    """
    ...
    """
    x, y = coords
    ax.text(x, y, letter, transform=ax.transAxes, size=size)


def annotateWithLetters(axes, size=20, coords=(-0.175, 1)):
    """
...
def boxNStripOrSwarmplot(
    ...
    the swarmplot.
    @param markercolor: A C{str} specifying the color for the dot in the swarmplot.
...
def plotMeansNErrors(
    xs, ys, yerrs, colors, ax, markers=None, markersize=20, fontsize=TEXT_SIZE
):
    """
...
def addHDIlinesToRidgePlot(ax, iData, varNames, hdi=(0.03, 0.97), colorDict=None):
    """
    ...
    for var, coords, _ in iDataListOrdered:
        color = colorDict[var] if colorDict and var in colorDict else "black"
        yCoord = yTicks[i]  # - 0.2
...
def ridgeForestPlot(
        ...
        for varName in varNames:
            ...
            for paramKey, displayLabel in PARAM_NAME_MAPPINGS.items():
                if varName in (
                    ...
                    varNamePattern = rf"{varName}$"
                elif varName in ("symptoms3",):
                    varNamePattern = (
                        r"symptoms3\[2\]"
                        if len(iData.posterior["symptoms3_dim"]) == 1
                        else rf"{varName}(?!\|)\["
                    )
                else:
                    varNamePattern = rf"{varName}(?!\|)\["
                if re.match(varNamePattern, paramKey):
                    yTickLabels.append(displayLabel)
        assert yTickLabels, (
            f"None of the provided variable names "
            f'({", ".join(varNames)!r} are recognized.'
        )
    yTickLabels = yTickLabels[::-1]  # Has to be reversed because of how plotting is
    # done.
    if not ax:
    ...
    if abs(leftLim) >= 10 or abs(rightLim) >= 10:
        step = 4
    elif abs(leftLim) >= 4 or abs(rightLim) >= 4:
        step = 2
    else:
        step = 1
    ...
    for x in xTickRange:
        if leftLim < x < rightLim:
            ...
            xTicks.append(x)
    ax.vlines(0, ymin=-1, ymax=18, color="gray", linewidth=0.5)
...
def setFontSize(ax, size=TEXT_SIZE):
    """
...
def _levellingOutcome(row, var="variant", outcome="agrdt", order=None):
    ...
    if order:
        ...
        order = row[var]
    gapSize = 0.02
    times = -1 if not row[outcome] else 1
    return int(row[outcome]) + times * gapSize + order * times * gapSize
...
def spaghettiPlotCategorical(
    ...
    """
    Create spaghetti plots (posterior predictions), with log10 viral load on the
    x-axis and agrdt result, n-antigen assay result (binary), or log10 coi n-antigen
    assay output on the y-axis.

    @param ax: A C{matplotlib} axes object for plotting.
    @param feature: A C{str} specifying the feature/variable to plot predictions for (
    e.g. for different SARS-CoV-2 variants.
    @param featureLevels: A C{iterable} specifying the levels of feature.
    @param predDict: A nested C{dict} containing posterior predictions for each C{
    featureLevel} along with data used for making predictions.
    @param df: A C{pd.DataFrame} containing the data used for fitting the statistical
    model.
    @param legendLoc: The position of the legend (x- and y-coordinate).
    @param palette: A C{dict} specifying the color mapping for each category in x.
    @param legend: A C{matplotlib object} of the legend to be shown in the plot.
    @param fontsize: A C{int} specifying the size of the font.
    @param showXLabel: A C{bool} indicating whether to show the x-axis label.
    @param showLegend: A C{bool} indicating whether to show the figure legend.
    @param vlVarName: A C{str} specifying the name of the viral load variable.
    @param vlRange: A C{tuple} specifying the range of log10 viral load values to show
    predictions for.
    @param outcome: A C{str} specifying the outcome variable.
    """

    assert outcome in ("agrdt", "Npos", "log10Coi")

    if feature == "variant":
        featurePalette = "variantCode" if isinstance(featureLevels[0], int) else feature
    elif feature in (
        "binDaysPostOnset",
        "binDaysPostOnset4",
        ...
        featurePalette = (
            "binDaysPostOnsetCode" if isinstance(featureLevels[0], int) else feature
        )
    else:
        featurePalette = feature
    palette = palette or getattr(PALETTE, featurePalette)
    ...
    for i, featureLevel in enumerate(featureLevels):
        ...
        ax.plot(
            vl,
            posteriorChainMean[idx][:, randIdcs],
            alpha=0.03,
            color=palette[featureLevel],
            linewidth=0.5,
        )
        ax.plot(
            vl,
    ...
    dfCopy = df[df[feature].isin(featureLevels)].copy()

    if outcome in ("agrdt", "Npos"):
        yMargin = 0.02 * len(featureLevels) + 0.02
        ax.set_ylim(-yMargin, 1 + yMargin)
        ax.set_xlim(vlRange[0], vlRange[1])
        order = dict(zip(featureLevels, range(len(featureLevels))))
        dfCopy[f"{outcome}Levels"] = dfCopy.apply(
            lambda row: _levellingOutcome(row, feature, outcome=outcome, order=order),
            axis=1,
        )
        yPlot = f"{outcome}Levels"
    else:
        ax.set_ylim(-0.2, 3.8)
        ax.set_xlim(vlRange[0], vlRange[1])
        yPlot = "log10Coi"
    ...
    if outcome == "agrdt":
        ax.set_ylabel("P(Ag-RDT=positive)")
    elif outcome == "Npos":
        ax.set_ylabel("P(assay=reactive)")
    elif outcome == "coi":
        ax.set_ylabel(LABEL.coi)
    elif outcome == "log10Coi":
        ax.set_ylabel(LABEL.log10Coi)

    xlabel = LABEL.vl if showXLabel else ""
    ax.set_xlabel(xlabel)

    if showLegend:
        ax.legend(handles=handles, loc=legendLoc, handlelength=2)
    else:
        ax.legend().remove()
...
def variantDominantTimesNumeric(df, timeVar="samplingMonth2"):
    ...
    # start date to the start date of the actual time period we are looking at (
    # i.e., to 1.12.2020).
...
def plotVariantDominantTimes(
    dfPos, palette, abbrvDictPaper, ax, timeVar="samplingMonth2", fontsize=TEXT_SIZE
):
    # Add colored bars indicating which variant was predominant at which time on top
    # of a plot with temporal x-axis.
    ...
    ax.set_xlim(*xlim)
    ax_2.set_xlim(*xlim)
    ax_2.tick_params(axis="both", which="both", length=0)

    setFontSize(ax_2, fontsize)


# Functions for creating figures used in the manuscript and thesis.
def plotFig1_A(
    dfFigure1_A,
    dfPos,
    timeVar,
    ...
    abbrvDictPaper,
    ax,
    showVariantTimes=True,
    fontSizePlot=TEXT_SIZE,
):
    ...
    if "samplingMonth2" not in statsDictTime:
        n = 1250
        # There is very little data for some categories, so we skip them
        newData = pd.DataFrame(
            {
                "samplingMonth2": np.concatenate(
                    list(
                        ...
                        if months in dfFigure1_A[timeVar].unique()
                    )
                )
            }
        )
        ...
        for samplingMonth2 in order.samplingMonth2:
            if samplingMonth2 not in dfFigure1_A[timeVar].unique():
                continue
            idx = newData.index[newData.samplingMonth2 == samplingMonth2].tolist()
            samples = iData.posterior.p.stack(sample=("chain", "draw"))[
                idx
            ].values.flatten()
    ...
    if xLabels:
        addFig1_x_labels(ax1, labels=xLabels, fontSizePlot=fontSizePlot)
...
def plotFig1_B(
    ...
    ax1.set_yticks(computeTicks(dfFigure1_B.vl, step=1))
...
def plotFig1_C(
    ...
    if showVariantTimes:
        assert len(ax) == 2
        ax2, ax1 = ax
        plotVariantDominantTimes(dfPos, palette, _ABBRVS, ax2, timeVar=timeVar)
    else:
        ax1 = ax
    # Vaccination status.
    ax_2 = ax1.twinx()
    ...
    y = []
    groups = dfFigure1_C.groupby(timeVar)
    for monthBin in xOrder:
        try:
            group = groups.get_group(monthBin)
            mean = group.immun2YN.sum() / len(group)
            y.append(mean)
        except KeyError:
            y.append(np.nan)
    ...
    if np.isnan(y[4]):
        ax_2.plot((2, 5), (y[2], y[5]), "--", alpha=0.5, color="gray", zorder=2)
    elif np.isnan(y[3]):
    ...
    # Formatting.
    xlim = (-0.5, 7.5) if timeVar == "samplingMonth2" else (-0.5, 14.5)
    ax1.set_ylim(-1.5, 250)
    ax1.set_xlim(*xlim)

    ax1.set_ylabel("Count")
    ax_2.set_ylabel("Multiply immunized fraction", rotation=270, labelpad=20)

    ax_2.grid(False)
    ax1.axes.get_xaxis().get_label().set_visible(False)
    ax_2.axes.get_xaxis().get_label().set_visible(False)
    ax_2.set_xticklabels([])
    ax_2.set_ylim(-0.01, 1.02)
    ...
    if xLabels is not None:
        addFig1_x_labels(ax1, labels=xLabels)

    # Remove x-tick marks
    ax1.tick_params(axis="x", which="both", bottom=False, top=False)


def addFig1_x_labels(ax, labels, fontSizePlot=TEXT_SIZE):
    ax.set_xlabel("\nSampling period")
...
def plotFig1_D(dfPos, timeVar, palette, labels, abbrvDictPaper, fontSizePlot, ax):
    ...
    addFig1_x_labels(ax, labels=labels, fontSizePlot=fontSizePlot)


def plotFig2_A(dfFigure2_A, label, palette, abbrvDictPaper, ax, fontSizePlot=TEXT_SIZE):
    ...
    bp, sp = boxNSwarmplot(
        ...
        ax=ax,
        palBoxplot=palette if isinstance(palette, str) else palette.variant,
        markersize=1.7,
        alphaBoxplot=0.7,
...
def plotFig2_B(
    dfFigure2_B, palette, label, order, abbrvDictPaper, ax, fontSizePlot=TEXT_SIZE
):
    # Use seaborns color parameter
    if palette:
        colors = [color for color in palette.variant.values() for _ in range(2)]
    else:
        colors = "none"
    ...
    xticklabels1 = list(
        chain.from_iterable(
            ("No Ag-RDT", "", "Ag-RDT") for _ in abbrvDictPaper["variant"].values()
        )
    )
    xticklabels2 = list(
        chain.from_iterable(
            ("", f"\n\n\n\n\n\n{variant}", "")
            for variant in abbrvDictPaper["variant"].values()
        )
    )
    xticks = sp.get_xticks()
    xticksNew = list(xtick + 0.001 for xtick in xticks)
    sp.set_xticklabels(xticklabels1, rotation=90, fontsize=10)
    for i, ticklabel in enumerate(xticklabels2):
        sp.text(
            xticksNew[i],
            0,
            ticklabel,
            ...
            ha="center",
            va="top",
            transform=sp.get_xaxis_transform(),
        )
    # Move x-axis label further down.
    sp.set_xlabel(label.variant, labelpad=35)
    setFontSize(sp, fontSizePlot)
...
def plotFig3_B(
    ...
    ax,
    immunSympDict=IMMUN_SYMP_DICT,
    immunSympPalette=IMMUN_SYMP_PAL,
    fontSizePlot=TEXT_SIZE,
):
...
def plotFigA2_A(
    ...
    order,
    ax,
    variantSympDict=VARIANT_SYMP_DICT,
    xLabels=False,
    fontSizePlot=TEXT_SIZE,
    markersize=2.5,
):
    ...
    setFontSize(ax, fontSizePlot)
    if xLabels:
        ax.set_xticklabels(variantSympDict.values(), rotation=45, ha="right")
...
def plotFigA2_B(
    ...
    ax,
    variantSympDict=VARIANT_SYMP_DICT,
    variantSympPalette=VARIANT_SYMP_PAL,
    fontSizePlot=TEXT_SIZE,
):
...
def plotFigA4_left(df, palette, label, ax, markersize=4, fontSizePlot=TEXT_SIZE):
    # Pre-VOC and Alpha.
    ...
    ax.set_ylim(3, 11)
    ax.set_title("Pre-VOC and Alpha", fontsize=fontSizePlot)
...
def plotFigA4_right(
    df, negPointsYCoords, palette, legend, ax, markersize=4, fontSizePlot=TEXT_SIZE
):
    # Delta and Omicron.
    ...
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_title("Delta and Omicron", fontsize=fontSizePlot)

    setFontSize(ax, fontSizePlot)


def plotDataPointsRegression(
    df, negPointsYCoords, ax1, ax2, pal, label, legend, fontSizePlot, fig, markersize=4
):
    plotFigA4_left(df, palette=pal, label=label, ax=ax1, markersize=markersize)
    plotFigA4_right(
        df, negPointsYCoords, palette=pal, legend=legend, ax=ax2, markersize=markersize
    )
    fig.text(0.55, 0, label.immun2YN, va="center", ha="center", fontsize=fontSizePlot)


def plotFigVlResult(df, label, markersize, ax):
    bp, sp = boxNSwarmplot(
        df,
        x="agrdt",
        y="vl",
    ...
    ax.set_yticks(computeTicks(df.vl, step=1))
    return ax


def plotFigVlTestline(df, label, markersize, ax):
    bp, sp = boxNSwarmplot(
        df,
        x="testline",
        y="vl",
        xLabel=label.testline,
        yLabel=label.vl,
        xCats=_ABBRVS["testline"],
        palBoxplot="none",
        markersize=markersize,
        alphaSwarmplot=0.8,
        ax=ax,
...
def plotSymptomHeatmap(df, ax=None, colOrder=None, cbar=True, title=""):
    """
    Plot a heatmap of the symptoms using the Jaccard index as a distance
    metric. The frequency of each symptom is shown on the diagonal.
    @param df: A C{pd.DataFrame} containing symptom columns.
    @param ax: A C{matplotlib} axes object for plotting.
    @param colOrder: The order according to which the symptoms should appear
    in the map.
    @param cbar: A C{bool} specifying whether to show the color bar next to
    the heatmap.
    @param title: The title of the figure.
    """
    symptomDisplayDict = symptomsDisplayDict()
    symptomCols = symptomColNames()
    if colOrder is None:
        cols = [col for col in symptomCols if col not in ("ill", "runnyNose")]
        # Use frequency of symptoms for the symptom order in the heatmap.
        colOrder = df[cols].mean().sort_values(ascending=False).index
    colOrder = list(colOrder)

    dfCorr = df[colOrder].copy()

    # See https://docs.scipy.org/doc/scipy/reference/generated/scipy.spatial.
    # distance.jaccard.html
    jacSim = makeSimilarityDf(dfCorr)

    # Calculate similarities for runny nose as there are a lot of NaN values
    # here because it only started to be in the questionnaire later on.
    dfRN = df[df.runnyNose.notna()].copy()
    rnCols = colOrder + ["runnyNose"]
    dfCorrRN = dfRN[rnCols].copy()

    jacSimRN = makeSimilarityDf(dfCorrRN)

    # Add frequencies of each symptom.
    for col in colOrder:
        jacSim.loc[col, col] = df[col].mean()

    # Add similarities for runny nose.
    # Add column.
    jacSim["runnyNose"] = jacSimRN["runnyNose"][:-1]
    # Add row.
    jacSim.loc["runnyNose"] = jacSimRN.loc["runnyNose"]
    # Add frequency.
    jacSim.loc["runnyNose", "runnyNose"] = dfRN["runnyNose"].mean()

    rnIndex = [
        i
        for i, col in enumerate(colOrder)
        if jacSim.loc["runnyNose", "runnyNose"] < jacSim.loc[col, col]
    ]
    rnIndex = rnIndex[-1] if rnIndex else 0

    colOrder.insert(rnIndex, "runnyNose")
    jacSim = jacSim[colOrder].copy()
    jacSim = jacSim.reindex(colOrder)

    # Rename columns and indexes.
    jacSim.rename(columns=symptomDisplayDict, inplace=True)
    jacSim.rename(index=symptomDisplayDict, inplace=True)

    annot = []
    mask = []
    for i, col1 in enumerate(colOrder):
        for j, col2 in enumerate(colOrder):
            annot.append(((df[col1] == 1) & (df[col2] == 1)).sum())
            mask.append(i < j)
    annot = np.array(annot).reshape((15, 15))
    mask = np.array(mask).reshape((15, 15))
    ...
    if ax is None:
        ...
        # Seaborn theme context needs to be created when creating the axis
        # object, that is outside this function.
        sns.heatmap(
            jacSim,
            square=True,
            cmap="Reds",
    ...
    if title:
        ax.set_title(title, size=TITLE_SIZE + 3)

    ax.set_xticklabels(ax.get_xticklabels(), fontsize=TEXT_SIZE + 3)
    ax.set_yticklabels(ax.get_yticklabels(), fontsize=TEXT_SIZE + 3)

    return jacSim


def saveFigure(fig, out, dpi=DPI):
